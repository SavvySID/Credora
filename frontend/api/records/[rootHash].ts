export default async function handler(
  ...
  try {
    const mod = process.env.VERCEL
      ? await import('../../api-lib/dist/handlers/recordByRoot.js')
      : await import('../../api-lib/handlers/recordByRoot');
    ...
    res.status(503).json({
      ...
      detail: error instanceof Error ? error.message : 'Record lookup handler failed',
