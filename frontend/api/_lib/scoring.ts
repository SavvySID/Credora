...
export const SCORING_MODEL = {
  ...
  trained: false,
  description:
    'Weighted sum of five normalised on-chain features. Reproducible from the inputs; no learned parameters.',
} as const;
...
export interface ScoreFactor {
  factor: keyof typeof FEATURE_WEIGHTS;
  impact: FactorImpact;
  weight: number;
  description: string;
  /** The raw value this factor was computed from, for auditability. */
  observed: string;
...
function logScale(value: number, saturation: number): number {
  if (value <= 0) return 0;
...
function impactFor(normalized: number): FactorImpact {
  if (normalized >= 0.6) return 'positive';
  if (normalized <= 0.35) return 'negative';
  return 'neutral';
}

export function scoreWallet(features: FeaturesDto, now = Date.now()): ScoreResult {
  ...
  factors.push({
    ...
    weight: FEATURE_WEIGHTS.balance,
  ...
  factors.push({
    ...
    weight: FEATURE_WEIGHTS.transaction_count,
  ...
  factors.push({
    ...
    weight: FEATURE_WEIGHTS.activity_recency,
  ...
  factors.push({
    ...
    weight: FEATURE_WEIGHTS.repayment_rate,
    ...
    description: hasRepaymentHistory
      ? `Repaid ${features.repayment.repaid} of ${
          features.repayment.repaid + features.repayment.defaulted
        } settled loans.`
  ...
  const riskLevel: ScoreResult['riskLevel'] =
    creditScore >= 700 ? 'High' : creditScore >= 400 ? 'Medium' : 'Low';
...
export function describeMethodology(result: ScoreResult): string {
  const parts = result.factors.map(
    (factor) =>
      `${factor.factor}=${factor.normalized.toFixed(3)}x${factor.weight}`,
  );
  return `${SCORING_MODEL.id}@${SCORING_MODEL.version} deterministic weighted sum: ${parts.join(
    ' + ',
  )} => ${result.creditScore}/1000`;
}
