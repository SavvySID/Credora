...
async function invoke(
  load: () => Promise<{ handle: (req: never, res: never) => Promise<void> }>,
  req: never,
  res: {
    headersSent?: boolean;
  ...
  try {
    ...
    await handle(req, res as never);
  } catch (error) {
    if (res.headersSent) return;
    unavailable(res, error instanceof Error ? error.message : fallback);
  }
}
...
export default async function handler(
  ...
  await invoke(
    () => import('../api-lib/handlers/creditProfile'),
    req as never,
    res,
    'Credit profile handler failed to load',
  );
}
